...
async function loadAll(){
  const [cfg, prop, anim, stk, gas, veh, med, vet] = await Promise.all([
    ...
    supabase.from('propiedades').select('*').order('id'),
    supabase.from('animales').select('*').order('id'),
    supabase.from('stock').select('*').order('id'),
    supabase.from('gastos').select('*').order('fecha',{ascending:false}),
    supabase.from('vehiculos').select('*').order('id'),
    supabase.from('medicamentos').select('*').order('id'),
    supabase.from('veterinarios').select('*').order('id'),
  ]);
...
async function guardarHectareas(){
  ...
  const {error} = await supabase.from('config').update({hectareas:val}).eq('id',1);
...
async function agregarAnimal(){
  ...
  const {data, error} = await supabase.from('animales').insert({tipo,nombre,edad,sexo}).select().single();
  if(error) return showConnError(error);
...
async function agregarStock(){
  ...
  const {data, error} = await supabase.from('stock').insert({cat:stockTabActual,item,cant,unidad,min}).select().single();
  if(error) return showConnError(error);
...
async function agregarGasto(){
  ...
  const {data, error} = await supabase.from('gastos').insert({categoria,tipo,descripcion,monto,fecha}).select().single();
  if(error) return showConnError(error);
...
async function agregarVehiculo(){
  ...
  const {data, error} = await supabase.from('vehiculos').insert({tipo,modelo,identificacion,estado}).select().single();
  if(error) return showConnError(error);
...
async function agregarMedicamento(){
  ...
  const {data, error} = await supabase.from('medicamentos').insert({nombre,cant,min,vencimiento}).select().single();
  if(error) return showConnError(error);
...
async function agregarVet(){
  ...
  const {data, error} = await supabase.from('veterinarios').insert({nombre,telefono,direccion}).select().single();
  if(error) return showConnError(error);
